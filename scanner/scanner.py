    ...
    for ticker in tickers:
        try:
            ...
            if resp.status_code == 404:
                # Ticker not found on iborrowdesk → unknown
            ...
            data = resp.json()
            # Response: { "ibkr": [ {"rate": 0.25, "available": 12345} ], "schwab": [...] }
            # Rate is annualised borrow fee %. available=0 means no locate.
            ibkr = data.get("ibkr", [])
            schwab = data.get("schwab", [])

            # Use IBKR first, fall back to Schwab
            for broker_data in [ibkr, schwab]:
                if broker_data:
                    latest = broker_data[-1]  # Most recent entry
...
def send_telegram_alert(candidates: list[dict], total_gappers: int = 0) -> None:
    ...
    if not a_plus:
        ...
        for c in monitor[:3]:
            ...
            lines.append(
                f"  • *{c['ticker']}* {borrow_icon}  {c.get('premarket_gap_pct', 0):.1f}% gap  "
    ...
    if monitor:
        ...
        lines.append(f"👀 Also monitoring: {', '.join(monitor_parts)}")
        lines.append("")
    if no_borrow:
...
def run_morning_mode():
    ...
    if a_plus:
        ...
        for c in a_plus:
            ...
            short_status = "✅ shortable" + borrow if c.get("shortable") else "⚠️ borrow unknown"
            print(f"\n  {'='*50}")
